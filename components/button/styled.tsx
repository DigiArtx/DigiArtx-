...
import { spacingSize } from "../views";
import { widthPixel } from "@utils/pxToDpConvert";
import { StyledTouchableOpacity } from "../views";
import { ButtonStylePropsExtra } from "./interface";
import { BoldText, RFFontSize, RFLineHeight } from "../text";
import { ColorValue, TouchableOpacity, ViewStyle } from "react-native";
import { widthPercentageToDP as wp } from "react-native-responsive-screen";

export const ButtonSolidView = styled(
  StyledTouchableOpacity
)<ButtonStylePropsExtra>(
  ({ backgroundColor, borderColor, width, borderWidth, paddingVertical }) => ({
    borderRadius: 8,
...
export const ThemedButtonOuter = styled.TouchableOpacity<{
  borderColor?: ColorValue;
}>(({ theme, borderColor = "#000" }) => ({
  borderTopWidth: 0,
  borderWidth: 2,
  borderRadius: 12,
...
export const ThemedButtonInner = styled.View<{
  ...
  isLoading?: boolean;
  borderColor?: ColorValue;
}>(({ outlined = false, theme, borderColor = "#000", isLoading }) => ({
  borderWidth: 2,
  borderRadius: 12,
  marginTop: -1.5,
  borderColor: borderColor,
...
export const IconButton = styled(StyledTouchableOpacity)(
  ({ minHeight = 50, minWidth = 50, ...rest }) => ({
    minHeight:
      typeof minHeight === "number" ? widthPixel(minHeight) : minHeight,
